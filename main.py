...
def main():
    ...
    print(f"Dynamic programming in all directions done in {toc(tt):.4f} [seconds]")
    ...
    WIN_SIZE = 5         # YOU MAY CHANGE THIS
    DISPARITY_RANGE = 60  # YOU MAY CHANGE THIS
    ...
    for i in range(1, 10):
        ...
        if i == 5:
            ...
            continue
        direction = i if i < 5 else i - 1
        plt.imshow(your_direction_to_vote[direction])
        plt.title(f"Direction {direction} {direction_arrow[direction]}")
        plt.axis("off")
    plt.show()
